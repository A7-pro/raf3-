...
def start(message):
    ...
    if user_id in BayaTi:
        ...
        bot.reply_to(message, "اهلا بك ، بوت رفع خارجي ", reply_markup=markup)
    else:
        bot.reply_to(message, "*تم وصل طلبك ، اذا م اشتركت كلم المطور ")
...
def request_approval(user_id, username):
    ...
    bot.send_message(Owner, f'''*هذا طلب تفعيل للبوت
...
def handle_approval(call):
    ...
    if call.data.startswith('Done_'):
        BayaTi.add(user_id)
        bot.send_message(user_id, "*تم وافقت عليك*")
        bot.send_message(Owner, "*• وافقت عليك")
    elif call.data.startswith("Reject_"):
        bot.send_message(user_id, "*• ما وافقت عليك*")

@bot.callback_query_handler(func=lambda call: True)
def handle_callback(call):
    user_id = call.message.chat.id
    if user_id not in BayaTi:
        bot.send_message(user_id, "للحين ماوافق المطور")
        return
...
def stop_spam(message):
    ...
    if user_id in user_data and user_data[user_id]['is_spamming']:
        ...
        bot.reply_to(message, "مو مشغل شي عشان اوقفه؟")
